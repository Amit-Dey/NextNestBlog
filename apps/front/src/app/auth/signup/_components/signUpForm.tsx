...
const SignUpForm = () => {
  ...
  return (
    <form action={action} className="w-full flex flex-col gap-4">
      {/* General message (e.g., success or top-level error) */}
      {!!state?.message && (
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          defaultValue={state?.data?.name} // Use optional chaining for state.data
        />
        {!!state?.errors?.name && (
      ...
      {/* Submit Button */}
      <SubmitButton className="w-full py-2.5 px-4 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition-colors duration-200 mt-2">
        Sign Up
      </SubmitButton>
    </form>
  );
